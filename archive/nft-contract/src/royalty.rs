...
impl NonFungibleTokenCore for Contract {
    ...
    fn nft_payout(&self, token_id: TokenId, balance: U128, max_len_payout: u32) -> Payout {
        ...
        for (k,v) in royalty.iter() {
            //get the key
            let key = k.clone();
            //only insert into the payout if the key isn't the token owner (add that at end)
            if key!= owner_id {
    ...
    fn nft_transfer_payout(
        ...
        //transfer token to the passed in receiver and get the previous token object back
        ...
        //payout to previous owner who gets 100% - total perpectual royalties
        payout_object.payout.insert(owner_id, royalty_to_payout(10000 - total_perpetual, balance_u128));
