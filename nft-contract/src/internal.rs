...
pub(crate) fn bytes_for_vote_cast(book_name: String) -> u64 {
    //the extra 4 bytes are coming from Borsch serialization to store the length of the string
    book_name.len() as u64 + 4 + size_of::<u64>() as u64
} 
...
//used to make sure the user attached at least 1 yoctoNEAR
...
pub(crate) fn refund_deposit(storage_used: u64) {
    ...
    //if the refund is greater than 1 yoctoNEAR, refund that amount
    if refund > 1 {
...
impl Contract {
    ...
    pub(crate) fn internal_add_token_to_owner(
        ...
        let mut tokens_set = self.tokens_per_owner.get(account_id).unwrap_or_else(|| {
            //if the account doesn't have any tokens, create new unordered set
            UnorderedSet::new(
    ...
    //remove a token from an owner (can't be called by CLI as internal method)
    pub(crate) fn internal_remove_token_from_owner(
        &mut self,
        account_id: &AccountId,
        token_id: &TokenId,
    ) {
        //get set of tokens the owner has
        ...
        //remove the token_id from the token set
    ...
    //transfers the NFT to the receiver_id (internal method not callable via CLI)
    pub(crate) fn internal_transfer(
        ...
        //remove the token from its curent owner's set
        self.internal_remove_token_from_owner(&token.owner_id, token_id);
        //we then add the token to the receiever_id's set
        ...
        //if there was some memo attached, log it
