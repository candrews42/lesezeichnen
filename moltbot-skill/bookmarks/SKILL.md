...
metadata: {"moltbot":{"requires":{"bins":["python3"],"env":["SUPABASE_URL","SUPABASE_ANON_KEY","LESEZEICHNEN_USER_ID"]},"emoji":"📚"}}
---
...
```bash
python {baseDir}/scripts/bookmarks.py mint \
  --book "Dune" \
  --author "Frank Herbert" \
  --rating 5 \
  --context "Vacation in Hawaii" \
...
Claude: [runs: python {baseDir}/scripts/bookmarks.py view --format pretty]
...
Claude: [runs: python {baseDir}/scripts/bookmarks.py update --book "Dune" --reflection "..."]
...
Claude: [runs: python {baseDir}/scripts/bookmarks.py update --book "The Left Hand of Darkness" --reflection "..."]
...
Claude: [runs: python {baseDir}/scripts/bookmarks.py update --last --reflection "..."]
...
python {baseDir}/scripts/bookmarks.py update --book "Dune" --reflection "..."

# By most recent
python {baseDir}/scripts/bookmarks.py update --last --reflection "..."

# By UUID (fallback)
python {baseDir}/scripts/bookmarks.py update --id <uuid> --reflection "..."
```
...
Claude: [runs: python {baseDir}/scripts/bookmarks.py stats --format pretty]
...
The CLI tool (`{baseDir}/scripts/bookmarks.py`) handles database operations:
...
# View with filters
python {baseDir}/scripts/bookmarks.py view [--filter rating:5] [--limit 10] [--format pretty]

# Update by book name (new!)
python {baseDir}/scripts/bookmarks.py update --book "Dune" --reflection "New thoughts"

# Update most recent (new!)
python {baseDir}/scripts/bookmarks.py update --last --reflection "New thoughts"

# Update by ID (fallback)
python {baseDir}/scripts/bookmarks.py update --id <uuid> --reflection "New thoughts"

# Stats
python {baseDir}/scripts/bookmarks.py stats [--format pretty]

# List recent (for selection)
python {baseDir}/scripts/bookmarks.py recent --limit 5
```
